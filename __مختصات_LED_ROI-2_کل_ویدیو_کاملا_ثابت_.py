...
class VideoAnalyzer:
    ...
    def __init__(self, video_path):
        ...
        # ==========================================================
        self.prev_gray = None
        # CUMULATIVE camera displacement relative to the first frame.
        # This is what makes the ENTIRE displayed video stay locked in place.
        self.camera_x = 0.0
        self.camera_y = 0.0
    ...
    def stabilize_frame(self, frame):
        """Lock the WHOLE video to the first frame.

        We estimate frame-to-frame camera translation, accumulate it, and
        compensate by the full accumulated displacement.  Unlike the old
        smoothing-only approach, this does not let the camera slowly wander
        across the screen.  The scene therefore stays at one fixed position
        while LED brightness/color changes are preserved.
        """
        try:
            ...
            if previous_points is not None and len(previous_points) >= 8:
                ...
                if current_points is not None and status is not None:
                    ...
                    good_previous = np.asarray(previous_points[valid], dtype=np.float32).reshape(-1, 2)
                    good_current = np.asarray(current_points[valid], dtype=np.float32).reshape(-1, 2)

                    if len(good_previous) >= 8:
                        movement = good_current - good_previous
                        # Robust median rejects changing LED pixels and other outliers.
                        dx_med = float(np.median(movement[:, 0]))
                        dy_med = float(np.median(movement[:, 1]))
                        distances = np.sqrt(
                            (movement[:, 0] - dx_med) ** 2 +
                            (movement[:, 1] - dy_med) ** 2
                        )
                        ...
                        if np.sum(inliers) >= 6:
                            dx = float(np.median(movement[inliers, 0]))
                            dy = float(np.median(movement[inliers, 1]))
                        else:
                            dx, dy = dx_med, dy_med
            ...
            # CRITICAL: accumulate the displacement.  The old code only
            # corrected the current motion, so the image could still drift.
            self.total_camera_x += dx
            self.total_camera_y += dy
            self.camera_x = self.total_camera_x
            self.camera_y = self.total_camera_y

            matrix = np.float32([
                [1, 0, -self.total_camera_x],
                [0, 1, -self.total_camera_y]
            ])
    ...
    def _make_map_panel(self, width, height):
        ...
        for gx,gy,gw,gh in map_objects:
            ...
            if selected_center is not None:
                live_cx, live_cy = selected_center
                fixed_cx, fixed_cy = int(round(cx_px)), int(round(cy_px))
                if (live_cx-fixed_cx)**2 + (live_cy-fixed_cy)**2 <= 20*20:
