...
class VideoAnalyzer:
    ...
    def stabilize_frame(self, frame):
        ...
        try:
            ...
            # OpenCV normally returns points as (N, 1, 2).  Flatten them
            # before indexing; otherwise a single/degenerate feature set can
            # produce the classic "index 1 is out of bounds for axis 1" error.
            good_previous = np.asarray(good_previous, dtype=np.float32).reshape(-1, 2)
            good_current = np.asarray(good_current, dtype=np.float32).reshape(-1, 2)
    ...
    def _print_selected_led(self):
        ...
        print()
        print('SELECTED LED - ROI INTENSITY PIXEL')
        print('ROI pixel    : X =', lx, ' Y =', ly)
        print('MAP position : column =', lx, ' row =', ly)
        if intensity is not None:
            print('Brightness   :', intensity, '/ 255')
        print('ROI size     : W =', rw, ' H =', rh)
        print('Detected box : X =', int(obj[0]-rx), ' Y =', int(obj[1]-ry), ' W =', int(obj[2]), ' H =', int(obj[3]), '(ROI-local)')
        print()
    ...
    def _make_coordinates_panel(self,width,height):
        ...
        if selected is None:
            ...
            cv2.putText(panel,'SELECTED LED - INTENSITY PIXEL',(18,y),cv2.FONT_HERSHEY_SIMPLEX,.56,(0,255,0),2,cv2.LINE_AA)
            y+=25
            brightness = None
            if self.current_roi is not None and 0 <= ly < self.current_roi.shape[0] and 0 <= lx < self.current_roi.shape[1]:
                brightness = int(cv2.cvtColor(self.current_roi, cv2.COLOR_BGR2GRAY)[ly,lx])
            lines=[
                f'ROI PIXEL    : X={lx}  Y={ly}',
                f'MAP POSITION : COL={lx}  ROW={ly}',
    ...
    def run(self):
        ...
        print("Click a GREEN LED in MAIN to inspect exact X/Y and MAP pixel position.")
