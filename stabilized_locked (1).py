...
class VideoAnalyzer:
    ...
    def __init__(self, video_path):
        ...
        # ==========================================================
        # STABILIZATION — REFERENCE LOCK + SUBPIXEL ECC
        # ==========================================================
        # The whole video is registered to ONE fixed reference frame.
        # This avoids the cumulative drift produced by frame-to-frame
        # stabilization.  A robust feature transform is used first and
        # ECC then refines it to sub-pixel accuracy.
        self.prev_gray = None
        self.reference_gray = None
        self.reference_points = None
        self.reference_frame = None
        self.stable_transform = np.eye(3, dtype=np.float32)
        self.camera_x = 0.0
        self.camera_y = 0.0
        self.smoothing = 0.88
        # HARD LOCK: movements below these limits are treated as camera noise.
        self.dead_zone = 1.50
        self.dead_zone_rotation_deg = 0.08
        self.dead_zone_scale = 0.003
        self.max_shift = 120.0
        self.max_rotation_deg = 3.0
        self.max_scale_change = 0.06
        self.ecc_iterations = 120
        self.ecc_epsilon = 1e-7
        self.stabilization_ready = False
        ...
        self.green_line_thickness = 2  # Automatic green LED box thickness (px)
        ...
        # LED layout lock: once ROI is selected, automatic green squares are
        # anchored to the reference ROI positions so their boxes never jitter.
        self.locked_led_objects = []
        self.locked_led_centers = []
    ...
    def _make_stabilization_reference(self, frame):
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        # A small Gaussian suppresses sensor noise but keeps LED/edge geometry.
        ref = cv2.GaussianBlur(gray, (0, 0), 0.8)
        self.reference_gray = ref.copy()
        self.reference_frame = frame.copy()
        self.reference_points = cv2.goodFeaturesToTrack(
            ref, maxCorners=1200, qualityLevel=0.005, minDistance=8, blockSize=7
        )
        self.prev_gray = gray.copy()
        self.stable_transform = np.eye(3, dtype=np.float32)
        self.camera_x = self.camera_y = 0.0
        self.stabilization_ready = True

    def _coarse_reference_transform(self, gray):
        if self.reference_gray is None or self.reference_points is None:
            return None
        if len(self.reference_points) < 8:
            return None
        current_points, status, _ = cv2.calcOpticalFlowPyrLK(
            self.reference_gray, gray, self.reference_points, None,
            winSize=(41, 41), maxLevel=5,
            criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 50, 0.001),
            flags=cv2.OPTFLOW_LK_GET_MIN_EIGENVALS, minEigThreshold=1e-5
        )
        if current_points is None or status is None:
            return None
        valid = status.reshape(-1) == 1
        src = self.reference_points.reshape(-1, 2)[valid]
        dst = current_points.reshape(-1, 2)[valid]
        if len(src) < 8:
            return None
        # ECC and warpAffine below use the same convention: this matrix maps
        # REFERENCE coordinates -> CURRENT coordinates, and WARP_INVERSE_MAP
        # is then used to bring CURRENT back onto the reference grid.
        M, inliers = cv2.estimateAffinePartial2D(
            src.astype(np.float32), dst.astype(np.float32),
            method=cv2.RANSAC, ransacReprojThreshold=1.0,
            maxIters=5000, confidence=0.9995, refineIters=20
        )
        if M is None or inliers is None or int(np.sum(inliers)) < 8:
            return None
        A = np.eye(3, dtype=np.float32)
        A[:2] = M.astype(np.float32)
        return A

    def _validate_stabilization_transform(self, M, frame_shape):
        ...
        h, w = frame_shape[:2]
        max_t = max(self.max_shift, 0.20 * max(h, w))
        return (abs(tx) <= max_t and abs(ty) <= max_t and
                abs(rotation) <= self.max_rotation_deg and
                abs(scale - 1.0) <= self.max_scale_change)

    def stabilize_frame(self, frame):
        """Hard reference-locked stabilization.

        The reference frame never moves. Tiny camera vibrations are ignored
        instead of being continuously followed. Larger real movements are
        corrected back toward the fixed reference grid.
        """
        try:
            ...
            gray = cv2.GaussianBlur(gray_raw, (0, 0), 0.8)

            if not self.stabilization_ready or self.reference_gray is None:
                self._make_stabilization_reference(frame)
                return frame.copy()

            coarse = self._coarse_reference_transform(gray)
            if coarse is None or not self._validate_stabilization_transform(coarse, frame.shape):
                # Tracking failure: HOLD the last valid lock. Do not jump.
                M = self.stable_transform[:2].copy()
            else:
                a, b, tx = [float(v) for v in coarse[0]]
                c, d, ty = [float(v) for v in coarse[1]]
                scale = 0.5 * (((a*a + c*c) ** 0.5) + ((b*b + d*d) ** 0.5))
                rotation = float(np.degrees(np.arctan2(c, a)))

                # ------------------------------------------------------
                # HARD DEAD-ZONE
                # Ignore tiny translations/rotation/scale changes.
                # This is what prevents the image from dancing by pixels.
                # ------------------------------------------------------
                tiny_translation = (abs(tx) < self.dead_zone and
                                    abs(ty) < self.dead_zone)
                tiny_rotation = abs(rotation) < self.dead_zone_rotation_deg
                tiny_scale = abs(scale - 1.0) < self.dead_zone_scale

                if tiny_translation and tiny_rotation and tiny_scale:
                    # SNAP to the exact reference grid.
                    self.stable_transform = np.eye(3, dtype=np.float32)
                    self.camera_x = 0.0
                    self.camera_y = 0.0
                else:
                    # For real movement, smooth the reference transform.
                    # Unlike frame-to-frame stabilization, this cannot drift.
                    alpha = float(np.clip(self.smoothing, 0.0, 0.98))
                    self.stable_transform = (
                        alpha * self.stable_transform + (1.0 - alpha) * coarse
                    ).astype(np.float32)
                    self.stable_transform[2] = [0.0, 0.0, 1.0]

                    if not self._validate_stabilization_transform(self.stable_transform, frame.shape):
                        # Never replace a good lock with a bad estimate.
                        self.stable_transform = np.eye(3, dtype=np.float32)
            ...
            # HARD SAFETY GATE: never show a corrupted stabilization result.
            bad = (
                stabilized is None or
                stabilized.size == 0 or
                stabilized.shape != frame.shape or
                not np.isfinite(stabilized).all() or
                float(np.mean(stabilized)) < max(1.0, float(np.mean(frame)) * 0.20)
            )

            if bad:
                # Keep the last transform instead of resetting to a moving frame.
                return cv2.warpAffine(
                    frame, self.stable_transform[:2],
                    (frame.shape[1], frame.shape[0]),
                    flags=cv2.INTER_LINEAR | cv2.WARP_INVERSE_MAP,
                    borderMode=cv2.BORDER_REFLECT101
                )

            self.camera_x = float(M[0, 2])
            self.camera_y = float(M[1, 2])
            self.prev_gray = gray_raw.copy()
            return stabilized

        except Exception as error:
            print('Stabilization error:', error)
            # On an exception, preserve the existing lock rather than showing
            # an unstabilized frame that could make the LED coordinates jump.
            try:
                return cv2.warpAffine(
                    frame, self.stable_transform[:2],
                    (frame.shape[1], frame.shape[0]),
    ...
    def _find_selected_led(self):
        if self.selected_led_center is None or not self.detected_objects:
            return None
    ...
    def _lock_led_layout_from_roi(self, roi, x1, y1):
        """Detect once on the reference ROI and keep LED geometry fixed."""
        found = self.filter_detections(self.detect_objects(roi))
        locked = []
        for rx, ry, rw, rh in found:
            locked.append([int(x1 + rx), int(y1 + ry), int(rw), int(rh)])
        self.locked_led_objects = locked
        self.locked_led_centers = [
            (int(x + w / 2), int(y + h / 2)) for x, y, w, h in locked
        ]
        return locked
    ...
    def _make_map_panel(self, width, height):
        ...
        selected=self._find_selected_led()
        for gx,gy,gw,gh in self.detected_objects:
            cx_px,cy_px=self._led_center_pixel((gx,gy,gw,gh))
            ...
            if selected is not None and list(map(int,selected)) == [int(gx),int(gy),int(gw),int(gh)]:
    ...
    def _compose_dashboard(self, display_frame):
        ...
        cv2.putText(main,f"Mode:{self.detection_mode}  Detected:{len(self.detected_objects)}  STABLE:REFERENCE+ECC  VideoZoom:x{self.video_zoom:.1f}  ROI-Zoom:x{self.main_zoom:.1f}",
        ...
        while True:
            ...
            if self.is_roi_selected and self.roi is not None:
                ...
                if x2>x1 and y2>y1:
                    ...
                    # Use the ROI geometry from the first/reference detection.
                    # Brightness is allowed to change, but the LED squares do not move.
                    self.detected_objects=[]
                    source_objects = self.locked_led_objects
                    if not source_objects:
                        source_objects = self._lock_led_layout_from_roi(roi, x1, y1)
                    for obj in source_objects:
                        if not self.is_deleted_object(obj):
                            self.detected_objects.append(list(obj))

                    # Assign LED-based coordinates using ONLY detections inside ROI.
                    self._build_led_grid_coordinates(self.detected_objects)
            ...
            # Emergency render fallback: the MAIN panel must never be black.
            if dashboard is None or dashboard.size == 0 or float(np.mean(dashboard)) < 0.5:
                dashboard = np.zeros((908,1608,3), dtype=np.uint8)
                fallback = self._fit_image(frame, 800, 450, cv2.INTER_AREA)
                dashboard[:450,:800] = fallback
                cv2.putText(dashboard, 'DISPLAY FALLBACK - RAW FRAME', (18, 35),
            ...
            if key==ord("q"):
                ...
                if new_roi[2]>0 and new_roi[3]>0:
                    ...
                    if rx2>rx1 and ry2>ry1:
                        ...
                        self.locked_led_objects=[]
                        self.locked_led_centers=[]
                        self._lock_led_layout_from_roi(self.frozen_roi, rx1, ry1)
                        self.frozen_detected_objects=[list(o) for o in self.locked_led_objects
                                                     if not self.is_deleted_object(o)]
                ...
                if ret:
                    ...
                    # Keep the immutable stabilization reference while stepping.
