...
class VideoAnalyzer:
    ...
    def mouse_callback(
        ...
        # The four-panel window is 640x360 per panel. Mouse actions
        # are accepted only on the VIDEO panel and converted back to
        # the original frame coordinates.
        if x >= 640 or y >= 360 or self.current_frame is None:
            return
    ...
    # FOUR-PANEL DISPLAY
    # ==========================================================
    def fit_panel(self, image, size, title):
        pw, ph = size
        canvas = np.zeros((ph, pw, 3), dtype=np.uint8)
        canvas[:] = (18, 18, 18)
        if image is None or image.size == 0:
            cv2.putText(canvas, title, (12, 28), cv2.FONT_HERSHEY_SIMPLEX,
                        0.65, (235, 235, 235), 2, cv2.LINE_AA)
            return canvas
        ih, iw = image.shape[:2]
        scale = min((pw-10)/iw, (ph-42)/ih)
        nw, nh = max(1, int(iw*scale)), max(1, int(ih*scale))
        interp = cv2.INTER_NEAREST if scale >= 1 else cv2.INTER_AREA
        resized = cv2.resize(image, (nw, nh), interpolation=interp)
        x = (pw-nw)//2
        y = 36 + max(0, (ph-36-nh)//2)
        canvas[y:y+nh, x:x+nw] = resized
        cv2.putText(canvas, title, (12, 27), cv2.FONT_HERSHEY_SIMPLEX,
                    0.65, (235, 235, 235), 2, cv2.LINE_AA)
        return canvas

    def build_zoom_panel(self, roi):
        if roi is None or roi.size == 0:
            return self.fit_panel(None, (640, 360), "ZOOM")
        zoom = cv2.resize(roi, None, fx=self.zoom, fy=self.zoom,
                          interpolation=cv2.INTER_NEAREST)
        # Clean, uniform thin green detection squares.
        for gx, gy, gw, gh in self.detected_objects:
            x1, y1 = self.current_roi_origin
            lx = int((gx-x1)*self.zoom); ly = int((gy-y1)*self.zoom)
            rw = max(1, int(gw*self.zoom)); rh = max(1, int(gh*self.zoom))
            cv2.rectangle(zoom, (lx,ly), (lx+rw,ly+rh), (0,255,0), 1)
        return self.fit_panel(zoom, (640, 360), f"ZOOM x{self.zoom:.1f}")

    def build_map_panel(self, roi):
        pw, ph = 640, 360
        panel = np.zeros((ph,pw,3), dtype=np.uint8); panel[:] = (22,22,22)
        cv2.putText(panel, "MAP", (12,27), cv2.FONT_HERSHEY_SIMPLEX, .65, (235,235,235), 2, cv2.LINE_AA)
        if roi is None or roi.size == 0: return panel
        h,w=roi.shape[:2]
        left,top,right,bottom=55,48,pw-25,ph-35
        sx=(right-left)/max(1,w); sy=(bottom-top)/max(1,h)
        # grid/axes
        cv2.line(panel,(left,bottom),(right,bottom),(150,150,150),1)
        cv2.line(panel,(left,top),(left,bottom),(150,150,150),1)
        cv2.putText(panel,"X",(right-14,bottom-7),cv2.FONT_HERSHEY_SIMPLEX,.45,(180,180,180),1,cv2.LINE_AA)
        cv2.putText(panel,"Y",(left+7,top+16),cv2.FONT_HERSHEY_SIMPLEX,.45,(180,180,180),1,cv2.LINE_AA)
        for i,(gx,gy,gw,gh) in enumerate(self.detected_objects,1):
            x1,y1=self.current_roi_origin
            lx=(gx-x1)*sx+left; ly=(gy-y1)*sy+top
            rw=max(2,gw*sx); rh=max(2,gh*sy)
            cv2.rectangle(panel,(int(lx),int(ly)),(int(lx+rw),int(ly+rh)),(0,255,0),1)
            cv2.putText(panel,f"{gx},{gy}",(int(lx)+3,max(top+12,int(ly)-2)),cv2.FONT_HERSHEY_SIMPLEX,.38,(0,255,0),1,cv2.LINE_AA)
        return panel

    def build_letter_panel_4(self, source_image):
        panel_w,panel_h=640,360
        panel=np.zeros((panel_h,panel_w,3),dtype=np.uint8); panel[:]=(22,22,22)
        cv2.putText(panel,f"LETTER VIEW: {self.selected_letter}",(12,27),cv2.FONT_HERSHEY_SIMPLEX,.65,(235,235,235),2,cv2.LINE_AA)
        left,top,right,bottom=55,48,panel_w-25,panel_h-25
        cw,ch=right-left,bottom-top
        glyph=np.zeros((ch,cw),dtype=np.uint8)
        scale=10.0
        (tw,th),base=cv2.getTextSize(self.selected_letter,self.letter_font,scale,self.letter_thickness)
        while (tw>cw-20 or th+base>ch-20) and scale>1:
            scale*=.9; (tw,th),base=cv2.getTextSize(self.selected_letter,self.letter_font,scale,self.letter_thickness)
        cv2.putText(glyph,self.selected_letter,(max(2,(cw-tw)//2),max(th+2,(ch+th)//2)),self.letter_font,scale,255,self.letter_thickness,cv2.LINE_AA)
        if source_image is not None and source_image.size:
            vid=cv2.resize(source_image,(cw,ch),interpolation=cv2.INTER_NEAREST)
            letter=cv2.bitwise_and(vid,vid,mask=glyph)
            panel[top:bottom,left:right]=letter
        cv2.line(panel,(left,bottom),(right,bottom),(150,150,150),1); cv2.line(panel,(left,top),(left,bottom),(150,150,150),1)
        cv2.putText(panel,"X",(right-14,bottom-7),cv2.FONT_HERSHEY_SIMPLEX,.45,(180,180,180),1,cv2.LINE_AA)
        cv2.putText(panel,"Y",(left+7,top+16),cv2.FONT_HERSHEY_SIMPLEX,.45,(180,180,180),1,cv2.LINE_AA)
        return panel

    def compose_four_panels(self, video, roi):
        a=self.fit_panel(video,(640,360),"VIDEO")
        b=self.build_zoom_panel(roi)
        c=self.build_map_panel(roi)
        d=self.build_letter_panel_4(roi if roi is not None else video)
    ...
    def run(self):
        ...
        while True:
            ...
            # SHOW FOUR PANELS
            # ==================================================
            combined_display = self.compose_four_panels(
                display_frame,
                self.current_roi if self.is_roi_selected else None
            )
            cv2.imshow("Main Video", combined_display)
