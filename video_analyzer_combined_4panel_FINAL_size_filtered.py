...
class VideoAnalyzer:
    # Display box sizing: keep map/manual markers uniform and easy to see.
    MAP_BOX_HALF = 6
    ...
    def __init__(self, video_path):
        ...
        # ----------------------------------------------------------
        # PIXEL SIZE FILTER
        # Only accept detections whose actual connected component is
        # within a normal pixel-sized range. Very tiny specks and
        # large blobs/objects are NOT considered pixels and receive
        # no green detection box.
        # ----------------------------------------------------------
        self.min_pixel_width = 2
        self.min_pixel_height = 2
        self.max_pixel_width = 16
        self.max_pixel_height = 16
        self.max_pixel_area = 180
        self.max_pixel_aspect = 2.0

        self.min_area = self.min_pixel_width * self.min_pixel_height
        self.max_area = self.max_pixel_area
    ...
    def detect_objects(self, image):
        ...
        for label in range(1, num_labels):
            ...
            # Reject components that are too tiny or too large to be
            # a real target pixel. This is based on the ACTUAL detected
            # component size, not the visual size of the drawing box.
            if w < self.min_pixel_width or h < self.min_pixel_height:
                continue

            if w > self.max_pixel_width or h > self.max_pixel_height:
                continue

            if area < self.min_area or area > self.max_pixel_area:
                continue

            aspect = max(w, h) / max(1.0, min(w, h))
            if aspect > self.max_pixel_aspect:
                continue
    ...
    def draw_manual_point(
        ...
        # Manual marker uses the same normal-sized square as the map
        # detection markers, rather than becoming oversized when zoomed.
        size = max(3, int(self.MAP_BOX_HALF * max(scale, 1.0)))
        ...
        cv2.rectangle(
            ...
            (local_x + size, local_y + size),
            (0, 0, 255),
            self.ZOOM_BOX_THICKNESS if scale > 1.0 else 2
        )
    ...
    def _draw_main_boxes(self, panel):
        ...
        for gx, gy, gw, gh in self.detected_objects:
            ...
            # Draw the real detected component bounds. Components that
            # are too small/large were already rejected by detect_objects().
    ...
    def _make_map_panel(self, width, height):
        ...
        for i,(gx,gy,gw,gh) in enumerate(self.detected_objects,1):
            ...
            # Every green map marker has exactly the same visible size.
            box_half = self.MAP_BOX_HALF
            cv2.rectangle(panel,
        ...
        for i,(px,py) in enumerate(self.manual_points,1):
            ...
            # Manual red marker is exactly the same size as green markers.
            box_half = self.MAP_BOX_HALF
            cv2.rectangle(panel,
                          (cx-box_half, cy-box_half),
                          (cx+box_half, cy+box_half),
    ...
    def _compose_dashboard(self, display_frame):
        ...
        cv2.putText(main,f"Mode:{self.detection_mode}  Detected:{len(self.detected_objects)}  Zoom:x{self.main_zoom:.1f}",
        ...
        if self.frozen_roi is not None and self.frozen_roi.size > 0:
            ...
            for gx, gy, gw, gh in self.frozen_detected_objects:
                ...
                if lx + gw < 0 or ly + gh < 0 or lx >= zsrc.shape[1] or ly >= zsrc.shape[0]:
                    continue
                # Thin green outline in the frozen zoom panel.
                cv2.rectangle(
                    zsrc,
                    (lx, ly),
                    (lx + max(int(gw), 4), ly + max(int(gh), 4)),
                    (0, 255, 0),
                    self.ZOOM_BOX_THICKNESS
                )
    ...
    def run(self):
        ...
        print("B Brightness | C Color | A Brightness+Color")
        ...
        while True:
            ...
            if self.is_roi_selected and self.roi is not None:
                ...
                if x2>x1 and y2>y1:
                    ...
                    for gx,gy,gw,gh in self.detected_objects:
                        cv2.rectangle(display_frame,(gx,gy),(gx+gw,gy+gh),(0,255,0),2)
                    for px,py in self.manual_points:
