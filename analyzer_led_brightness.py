...
class VideoAnalyzer:
    ...
    def __init__(self, video_path):
        ...
        self.led_percentile = 98.2
        self.led_min_gray = 135
        self.led_local_contrast = 12
        self.led_peak_radius = 4
        self.led_merge_distance = 7
        self.led_box_size = 8
        self.led_blur_sigma = 5.0
        self.led_core_percentile = 99.0
    ...
    def _detect_led_centers(self, image):
        """
        Find LED cores as local luminance maxima.

        The detection point is the brightest core, while the displayed
        green square always uses one fixed size. Therefore the amount
        of light bloom does not change the pixel/square dimensions.
        """
        if image is None or image.size == 0:
            return []
        ...
        contrast = gray_f - background

        percentile = float(np.percentile(
            gray_f, self.led_percentile
        ))
        intensity_threshold = max(
            float(self.led_min_gray), percentile
        )
        ...
        # +/-1 handles small compression/noise variations.
        is_peak = gray.astype(np.int16) >= (
            local_max.astype(np.int16) - 1
        )

        core_percentile = float(np.percentile(
            gray_f, self.led_core_percentile
        ))
        core_threshold = max(
            intensity_threshold, core_percentile
        )

        strong_peak = (
            is_peak &
            (gray_f >= self.led_min_gray) &
            (contrast >= self.led_local_contrast)
        )
        very_bright = is_peak & (gray_f >= core_threshold)

        candidate_mask = np.where(
            strong_peak | very_bright, 255, 0
        ).astype(np.uint8)
        ...
        for label in range(1, num_labels):
            ...
            ix = int(np.clip(round(cx), 0, gray.shape[1] - 1))
            iy = int(np.clip(round(cy), 0, gray.shape[0] - 1))

            # Refine the center to the brightest actual pixel.
            ...
            candidates.append((
                int(gray[iy, ix]),
                float(contrast[iy, ix]),
                ix,
                iy
            ))

        # Strongest cores first, then suppress duplicate detections
        # belonging to the same light bloom.
        candidates.sort(
        ...
        for brightness, local_score, cx, cy in candidates:
            duplicate = any(
                (cx - ox) ** 2 + (cy - oy) ** 2 <= merge_d2
    ...
    def _make_map_panel(self, width, height):
        ...
        for i,(gx,gy,gw,gh) in enumerate(self.detected_objects,1):
            ...
            cv2.rectangle(panel,(cx-bw//2,cy-bh//2),(cx+bw//2,cy+bh//2),(0,255,0),1)
